...
from scipy.special import gammaln  # no longer used but harmless
import warnings
...
# =============================================================================
# Maximum Likelihood helpers and models (JAX-based)
# =============================================================================

def sigmoid(z):
    # numpy version used in prediction curves
    return 1.0 / (1.0 + np.exp(-z))

def rectified_logistic(x01, a, b, g, h, v):
    # numpy version used for plotting / prediction (outside JAX)
...
# ------------------- JAX rectified logistic (for likelihoods) -----------------

def jax_rectified_logistic(x01, a, b, g, h, v):
    inner = b * (x01 - a) - jnp.log(h / v)
    sig = 1.0 / (1.0 + jnp.exp(-inner))
    return g + jnp.maximum(0.0, (-v + (h + v) * sig))

# =============================================================================
# Gamma model 1: constant k (JAX likelihood with vmap)
# =============================================================================

def _point_nll_k(theta, x_i, y_i):
    """
    Negative log likelihood for a single data point under the constant-k gamma model.
    theta = [a, b, g, h, v, log_k]
    """
    a, b, g, h, v, log_k = theta

    mu = jax_rectified_logistic(x_i, a, b, g, h, v)
    mu = jnp.clip(mu, 1e-6, jnp.inf)
    y = jnp.clip(y_i, 1e-6, jnp.inf)

    k = jnp.exp(log_k)
    theta_scale = mu / k  # mean = k * theta_scale = mu

    # log p(y) = (k - 1) * log y - y/theta_scale - k * log(theta_scale) - log Gamma(k)
    log_pdf = (k - 1.0) * jnp.log(y) - (y / theta_scale) - k * jnp.log(theta_scale) - jsp.special.gammaln(k)
    return -log_pdf  # negative log likelihood for one point

_point_nll_k_vmap = jax.vmap(_point_nll_k, in_axes=(None, 0, 0))

@jax.jit
def gamma_negloglik_jax(theta, x01, y):
    """
    JAX gamma negative log likelihood for the constant-k model.
    Uses vmap over data points.
    """
    theta_j = jnp.asarray(theta)
    x01_j = jnp.asarray(x01)
    y_j = jnp.asarray(y)
    per_point_nll = _point_nll_k_vmap(theta_j, x01_j, y_j)
    return jnp.sum(per_point_nll)

def gamma_negloglik(theta, x01, y):
    """
    SciPy-compatible wrapper.
    Calls the JAX likelihood and returns a Python float.
    """
    nll = float(gamma_negloglik_jax(theta, x01, y))
...
# =============================================================================
# Gamma model 2: alpha = mu * beta (c1, c2) (JAX likelihood with vmap)
# =============================================================================

def _point_nll_c1c2(theta, x_i, y_i):
    """
    Negative log likelihood for a single data point under the c1/c2 gamma model.
    theta = [a, b, g, h, v, log_c1, log_c2]
    """
    ...
    beta = jnp.clip(beta, 1e-6, jnp.inf)

    # alpha = mu * beta
    alpha = mu * beta
    alpha = jnp.clip(alpha, 1e-6, jnp.inf)

    # Gamma(alpha, beta) with rate parameterization:
    # log p(y) = (alpha - 1) * log y - beta * y + alpha * log(beta) - log Gamma(alpha)
    log_pdf = (alpha - 1.0) * jnp.log(y) - beta * y + alpha * jnp.log(beta) - jsp.special.gammaln(alpha)
    return -log_pdf

_point_nll_c1c2_vmap = jax.vmap(_point_nll_c1c2, in_axes=(None, 0, 0))

@jax.jit
def gamma_negloglik_c1c2_jax(theta, x01, y):
    """
    JAX gamma negative log likelihood for the c1/c2 model.
    Uses vmap over data points.
    """
    theta_j = jnp.asarray(theta)
    x01_j = jnp.asarray(x01)
    y_j = jnp.asarray(y)
    per_point_nll = _point_nll_c1c2_vmap(theta_j, x01_j, y_j)
    return jnp.sum(per_point_nll)

def gamma_negloglik_c1c2(theta, x01, y):
    """
    SciPy-compatible wrapper.
    Calls the JAX c1/c2 likelihood and returns a Python float.
    """
    nll = float(gamma_negloglik_c1c2_jax(theta, x01, y))
    if not np.isfinite(nll):
        return 1e12
    return nll
...
# =============================================================================
# Single init runners (unchanged logic, just calling JAX likelihoods)
# =============================================================================
...
# =============================================================================
# Fit rectified logistic: constant k model
# (no multiprocessing, loop over inits; JAX inside likelihood)
# =============================================================================
...
def fit_rectified_logistic(x, y, n_inits=n_inits):
    ...
    for args in args_list:
        results.append(run_single_init(args))
...
# =============================================================================
# Fit rectified logistic: alpha = mu * beta (c1, c2) model
# =============================================================================
...
def fit_rectified_logistic_c1c2(x, y, n_inits=n_inits):
    ...
    results = []
    for args in args_list:
        results.append(run_single_init_c1c2(args))
    ...
    print("\n=== Fitting summary (MLE c1c2 model) ===")
    for i, r in enumerate(results):
        if r["success"]:
            print(f"Init {i+1}: RMSE={r['rmse']:.4f}, R²={r['r2']:.4f}")
        else:
...
def main():
    ...
    output_path = os.path.join(
        output_dir,
        f"paired_curves_all_muscles_n={n_inits}_MLE_k_vs_c1c2_JAX.pdf"
    )
    make_pdf(figures=all_figures, output_path=output_path)
    print(f"\nSaved combined PDF for all muscles (MLE k vs c1c2, JAX) to:\n{output_path}\n")
