...
#%%
#  Maximum Likelihood for Gamma noise (JAX-based likelihood, SciPy optimizer)

def sigmoid(z):
    return 1.0 / (1.0 + np.exp(-z))

def rectified_logistic(x01, a, b, g, h, v):
    # numpy version used for diagnostics and prediction
...
# JAX version of rectified logistic used in the likelihood
...
# pointwise negative log likelihood for a single data point
def _point_nll(theta, x_i, y_i):
    a, b, g, h, v, log_k = theta

    mu = jax_rectified_logistic(x_i, a, b, g, h, v)
    mu = jnp.clip(mu, 1e-6, None)
    y_clipped = jnp.clip(y_i, 1e-6, None)

    k = jnp.exp(log_k)
    theta_scale = mu / k

    log_pdf = (k - 1.0) * jnp.log(y_clipped) \
              - (y_clipped / theta_scale) \
              - k * jnp.log(theta_scale) \
              - jsp.special.gammaln(k)

    # return negative log likelihood for one point
    return -log_pdf

# vmap over data points for speed and purity
_point_nll_vmap = jax.vmap(_point_nll, in_axes=(None, 0, 0))
...
def gamma_negloglik_jax(theta, x01, y):
    ...
    per_point_nll = _point_nll_vmap(theta_j, x01_j, y_j)
    return jnp.sum(per_point_nll)

def gamma_negloglik(theta, x01, y):

    nll = gamma_negloglik_jax(theta, x01, y)
    nll_val = float(nll)
...
def main():
    ...
    output_path = os.path.join(output_dir, f"paired_curves_all_muscles_n={n_inits}_MLE_JAX.pdf")
