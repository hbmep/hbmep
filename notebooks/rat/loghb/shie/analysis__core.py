...
from hbmep.notebooks.rat.analysis import load_shie as load
...
def threshold_analysis(model_dir, correction=False, fig=None, dump=False):
    ...
    a = posterior[site.a].copy(); checknans(a)
    a = np.mean(a, axis=0, keepdims=True); print(a.shape)
    a = a.reshape(*a.shape[:-3], -1, a.shape[-1]); print(a.shape)
...
def main():
    ...
    out += [threshold_analysis(model_dir, dump=True)[0][0] for model_dir in model_dirs]
    ...
    out += [estimation_analysis(model_dir)[0][0] for model_dir in model_dirs]
    ...
    out += [figure(mdirs, correction=True)[0][0] for mdirs in model_dirs]
    return out
