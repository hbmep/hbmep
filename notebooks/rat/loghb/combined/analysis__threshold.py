...
SEPARATOR = "___"
BUILD_DIR = "/home/vishu/reports/hbmep/notebooks/rat/loghb/combined_data/out-threshold"
...
def arrange(run_id, model_dir):
    ...
    if run_id in {"diam", "radii", "vertices"}:
        ...
        if "ground" in run_id:
            experiment = [(u, v) for u, v in experiment if v[0].split("-")[0] == ""]
        else:
            raise ValueError("Logic for bipolar not implemented for C_SMA_LAR")
        experiment = sorted(experiment, key=lambda x: (x[1][0], x[1][1], x[1][2]))
    ...
    posterior = {u: v[..., idx, :] for u, v in posterior.items() if u in named_params}
    positions = list(zip(range(len(positions)), positions))

    df = df[df[model.features[1]].isin(idx)].reset_index(drop=True).copy()
    df[model.features[1]] = df[model.features[1]].map(dict(zip(idx, range(len(positions)))))
...
def main():
    ...
    out += [run(run_id, model_dir, correction=True, dump=True)[0][0] for run_id in run_ids]
    output_path = os.path.join(BUILD_DIR, "out.pdf")
    make_pdf(out, output_path)

    # run_id = "lat-big-ground"
    # run(run_id, model_dir, correction=True)
