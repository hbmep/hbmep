...
class GeneralizedLogistic(Baseline):
    ...
    def _model(self, intensity, participant, feature0, feature1, response_obs=None):
        ...
        with numpyro.plate("n_participant", n_participant, dim=-1):
            ...
            a_mean = numpyro.sample(
                site.a_mean,
                dist.TruncatedDistribution(dist.Normal(100, 50), low=0)
            )
            a_scale = numpyro.sample(site.a_scale, dist.HalfNormal(50))

            b_scale = numpyro.sample(site.b_scale, dist.HalfNormal(2))
        ...
        # Model
        mean = \
            l[feature1, feature0, participant] + \
            (l[feature1, feature0, participant] + h[feature1, feature0, participant]) / \
            jnp.power(
                1 + jnp.exp(-b[feature1, feature0, participant] * (intensity - a[feature1, feature0, participant])),
    ...
    def plot(self, df: pd.DataFrame, posterior_samples: dict, encoder_dict: dict = None):
        ...
        for i, c in enumerate(combinations):
            ...
            if encoder_dict is None:
                ...
                c1 = temp_df[FEATURES[0]].unique()[0]
                c2 = encoder_dict[FEATURES[1]].inverse_transform(np.array([c[1]]))[0]
