...
class Estimation(BaseModel):
    ...
    def circ_est_mvn_rl_nov_masked_gshared(self, intensity, features, response=None, **kw):
        num_data = intensity.shape[0]
        num_features = np.max(features, axis=0) + 1
        ...
        a_fixed = pyro.sample("a_fixed", dist.Normal(5., 5.))
    ...
    def circ_est_mvn_rl_nov_masked_gc1c2shared(self, intensity, features, response=None, **kw):
        num_data = intensity.shape[0]
        num_features = np.max(features, axis=0) + 1
...
class HB(BaseModel):
    ...
    def hb_l4_masked(self, intensity, features, response=None, **kw):
        # Used for running the logistic-4 model independently on each muscle
        num_data = intensity.shape[0]
        num_features = np.max(features, axis=0) + 1
    ...
    # def hb_l4_masked_gshared(self, intensity, features, response=None, **kw):
    #     num_data = intensity.shape[0]
    #     num_features = np.max(features, axis=0) + 1
    ...
    def size_all_hb_mvn_l4_masked(self, intensity, features, response=None, **kw):
        num_data = intensity.shape[0]
        num_features = np.max(features, axis=0) + 1
