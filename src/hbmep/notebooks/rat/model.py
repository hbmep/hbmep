...
class Estimation(BaseModel):
    ...
    def lat_est_mvn_reference_rl_masked(self, intensity, features, response=None, **kw):
        ...
        a_fixed_loc = pyro.sample("a_fixed_loc", dist.Normal(5., 5.))
        a_fixed_scale = pyro.sample("a_fixed_scale", dist.HalfNormal(5.))
        # rho_fixed = pyro.sample("rho_fixed", dist.LKJ(num_features[-1] * self.num_response, 1.))
        # rho_fixed = pyro.sample("rho_fixed", LKJ(num_features[-1] * self.num_response, 1.))
        ...
        # key = random.key(0)
        # arr = random.randint(key, (2, 2), minval=1, maxval=9)
        # arr = np.array(arr)
        # t = np.block([[arr, arr], [arr, arr]])
        # t = np.block([[arr, zeros], [zeros, arr]])
        # print(arr)
        # print(t)
        ...
        with pyro.plate("num_delta", num_delta):
            a_delta_loc = pyro.sample("a_delta_loc", dist.Normal(0., 5.))
            a_delta_scale = pyro.sample("a_delta_scale", dist.HalfNormal(5.))
            # rho_delta = pyro.sample("rho_delta", dist.LKJ(num_features[-1] * self.num_response, 1.))
            # rho_delta = pyro.sample("rho_delta", LKJ(num_features[-1] * self.num_response, 1.))
